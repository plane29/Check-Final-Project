...
public class Main extends JPanel implements KeyListener, MouseListener{
    ...
    public void paintComponent(Graphics g) { //main idea of paint component taken from keyboard spheres
        ...
        if(Area.myInventory.lockSolved && Area.myInventory.pickSolved && r.currentArea.num == 9){
            for(int i = 1; i<r.areas.length;i++){
                r.areas[i].myHint = new Hint(WIDTH, HEIGHT, "You have all you need to unlock the door.");
            }
            r.areas[0].puzzleSolved();
            r.areas[0].myHint = new Hint(WIDTH,HEIGHT, "Congrats! You have escaped.  Feel free to look around.");
            r.areas[0].hClicked = true;
        }
        r.drawRoom(g);
        if(mouseInLeft){
            r.drawLeftArrow(g);
