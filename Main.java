...
public class Main extends JPanel implements KeyListener, MouseListener{
    public static int WIDTH = 960; //width of screen
    public static int HEIGHT = 640; //height of screen
    public Graphics g; //instance of graphics
    ...
    public boolean inMyArea;  //boolean to track if the user is in an area
    ...
    public boolean mouseInBottom;  //(incomplete) boolean to track if the user hovers mouse over bottom hitbox
    ...
    public static void listenToMouse(){
        try{  //try and catch if the mouse is out of screen don't want a nullpointer so we send to middle of screen
            play.p = MouseInfo.getPointerInfo().getLocation();
        }
        catch(NullPointerException e){
            play.p = new Point(WIDTH/2,HEIGHT/2);
        }

        play.mouseLocation =  new Pair(play.p.x,play.p.y -50);  //for some reason the initial mouse y location is 50 so we have to adjust for that
